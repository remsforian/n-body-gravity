...
def update(i): #updates the position of the bodies on scatter plot(s) (it is currently putting all the frames on top of each other) 
	ax.scatter.set_offsets(float(x1[i]),float(y1[i]),float(z1[i]),c='r')
	ax.scatter.set_offsets(float(x2[i]),float(y2[i]),float(z2[i]),c='b')
	ax.scatter.set_offsets(float(x3[i]),float(y3[i]),float(z3[i]),c='g')
	return ax
